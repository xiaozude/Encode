...
const unsigned char R_S_BOX[16][16] = {
	{
		0x52, 0x09, 0x6A, 0xD5,
		0x30, 0x36, 0xA5, 0x38,
		0xBF, 0x40, 0xA3, 0x9E,
...
{
	unsigned char buf[4][44];
	for (int i = 0; i < 4; i++) {
		memcpy(buf[i], key0[i], 4);
	}
	
	for (int j = 4; j < 44; j++) {
		for (int i = 0; i < 4; i++) {
			buf[i][j] = buf[i][j - 1] ^ buf[i][j - 4];
		}
		if (j % 4 != 0) continue;
		for (int i = 0; i < 4; i++) {
			buf[(i + 3) % 4][j] = buf[i][j - 1];
		}
		for (int i = 0; i < 4; i++) {
			unsigned char x = (buf[i][j] >> 4) & 0x0F;
			unsigned char y = buf[i][j] & 0x0F;
			buf[i][j] = S_BOX[x][y] ^ buf[i][j - 4] ^ RCON[i][j / 4 - 1];
		}
	}

	for (int j = 0; j < 44; j++) {
		for (int i = 0; i < 4; i++) {
			key[j / 4][i][j % 4] = buf[i][j];
		}
	}
}
...
void SubBytes(unsigned char data[4][4], const unsigned char s_box[16][16])
{
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			unsigned char x = (data[i][j] >> 4) & 0x0F;
			unsigned char y = data[i][j] & 0x0F;
			data[i][j] = s_box[x][y];
		}
	}
}
...
void ShiftRows(unsigned char data[4][4], bool flag)
{
	...
	for (int i = 0; i < 4; i++) {
		memcpy(buf, data[i], 4);
		for (int j = 0; j < 4; j++) {
			if (flag) data[i][j] = buf[(j + i) % 4];
			else data[i][j] = buf[(j - i + 4) % 4];
		}
	}
}
...
void AES(unsigned char data[4][4], unsigned char key[11][4][4], bool mode)
{
	if (mode) {
		...
		for (int k = 1; k <= 10; k++) {
			SubBytes(data, S_BOX);
			ShiftRows(data, true);
			if (k < 10) MixColumns(data, M);
			AddRoundKey(data, key[k]);
		}
	} else {
		AddRoundKey(data, key[10]);
		for (int k = 9; k >= 0; k--) {
			SubBytes(data, R_S_BOX);
			ShiftRows(data, false);
			if (k > 0) {
				MixColumns(data, R_M);
				MixColumns(key[k], R_M);
			}
			AddRoundKey(data, key[k]);
		}
	}
